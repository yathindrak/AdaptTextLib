...
class EnsembleTrainer(Trainer):
    ...
    def retrieve_classifier(self):
        ...
        prediction_tensors_fwd, prediction_tensors_target_fwd = self.__learn_clas_fwd.get_preds(DatasetType.Valid,
                                                                                                ordered=True)
        prediction_tensors_bwd, prediction_tensors_target_bwd = self.__learn_clas_bwd.get_preds(DatasetType.Valid,
                                                                                                ordered=True)
        ...
        ensemble_databunch = (TabularList
                              .from_df(ensemble_dataframe, cat_names=[], cont_names=column_names,
                                       procs=tabular_processes)
                              .split_by_rand_pct(valid_pct=valid_percentage, seed=42)
                              .label_from_df(cols="target")
                              .databunch())

        learn = tabular_learner(ensemble_databunch, layers=layer_set, ps=dropout_probs, metrics=metrics,
                                emb_drop=embedding_layer_dropout_prob)
