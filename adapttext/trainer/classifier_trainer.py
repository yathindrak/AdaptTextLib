...
class ClassifierTrainer(Trainer):
    ...
    def train(self, grad_unfreeze=True):
        ...
        learn.fit_one_cycle(11, lr, callbacks=[SaveModelCallback(learn,monitor='error_rate',mode='min'), ReduceLROnPlateauCallback(learn, factor=0.8)])

        if grad_unfreeze:
            learn.freeze_to(-2)
            learn.fit_one_cycle(8, lr,
                                callbacks=[SaveModelCallback(learn,monitor='error_rate',mode='min'), ReduceLROnPlateauCallback(learn, factor=0.8)])
        ...
        learn.unfreeze()
        learn.fit_one_cycle(8, max_lr=slice(3e-5, 3e-4), callbacks=[SaveModelCallback(learn,monitor='error_rate',mode='min'),
