...
class ClassifierTrainer(Trainer):
  ...
  def train(self):
    # config = dict(emb_sz=400, n_hid=1550, n_layers=4, pad_token=1, qrnn=False, output_p=0.25, hidden_p=0.1, input_p=0.2, embed_p=0.02, weight_p=0.15)
    # trn_args = dict(bptt=70, alpha=2, beta=1,max_len=1400)
    ...
    config = dict(emb_sz=size_of_embedding, n_hid=num_of_hidden_neurons, n_layers=num_of_layers,
                  input_p=dropout_probs['input'], output_p=dropout_probs['output'], hidden_p=dropout_probs['hidden'],
                  embed_p=dropout_probs['embedding'], weight_p=dropout_probs['weight'], pad_token=1, qrnn=False)

    learn = self.retrieve_classifier(self.data, config=config, drop_multi_val=self.drop_mult, metrics=[error_rate, accuracy])
